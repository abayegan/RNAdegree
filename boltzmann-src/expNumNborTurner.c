...
int main(int argc, char *argv[]){
	...
	char * engFlag = "turner04"; 
	int VERBOSE = 0;
	double TEMP=37.;
	const char turner99_eng[] = "rna_turner1999.par";
	const char turner04_eng[] = "rna_turner2004.par";
	const char andronesco07_eng[] = "rna_andronescu2007.par";
	const char * ENERGYFOLDER = "energy_params";
	int MS1 = 0 ;
	/*---------------------------------------------------
			...
			if (argv[i][0] == '-')
				switch (argv[i][1]){
					...
					case 'v':
						...
						break;
					case 'm':
						if(!strcmp(argv[i],"-ms1"))
							MS1=1;
						else if(!strcmp(argv[i],"-ms2"))
							MS1=0;
						else
							dispUsage(argv[0]);
						break;
	...
	if(!strcmp(engFlag,"turner99"))
		strcat(execPath,turner99);
	else if (!strcmp(engFlag,"turner04"))
		strcat(execPath , turner04);
	else if (!strcmp(engFlag,"andronescu07"))
		strcat(execPath , andronescu07);
	else
		dispUsage(argv[0]);
	read_parameter_file(execPath); //use turner99 energy model
	...
	{	
		...
		mcCaskill_ms2(sequence, Z, ZB, ZM1, ZM);
		...
		mcCaskillQ_ms2(Q,QB,QM1,QM,Z,ZB,ZM1,ZM,EL,ER1,G,F,sequence);
		expNumNhbors = Q[1][seqlen]/Z[1][seqlen];
		expNumNhborsNorm = expNumNhbors/seqlen;
		if(VERBOSE)
			printf("RNA Sequence:%s\nExpected degree(MS2):%lf\nNormalized expected degree(MS2):%lf\nPartition function(MS2):%lf\nTotal number of structures:%lf\n",
	...
	else if (MS1)
	{
		...
		if (VERBOSE)
		{
			...
			totalNumStr = Z[seqlen][1];
			printf("RNA sequence:%s\n",sequence+1);
			printf("Expected degree(MS1):%lf\n", expNumNhbors);
			printf("Normalized expected degree(MS1):%lf\n", expNumNhborsNorm);
			printf("Partition function(MS1):%lf\n",totalpar);
			printf("Total number of structures:%lf\n",totalNumStr);
			//printf("Q[1][n]:%lf\n",Q[1][seqlen]);
			//  printf("Q[n][1]:%lf\n",Q[seqlen][1]);
		}
		else
